...
static Cap *build_resolution_cap(Knot *knot, int r)
{
    ...
    for (int k = 0; k < m; k++) {
        for (int i = 0; i < 4; i++) {
            if (knot->pd[k][i] > max_edge)
                max_edge = knot->pd[k][i];
        }
    }

    int *first_port = (int *)malloc((size_t)(max_edge + 1) * sizeof(int));
    if (!first_port) {
        Cap_free(cap);
        return NULL;
    }
    ...
    int *external_mate = (int *)malloc((size_t)cap_size * sizeof(int));
    if (!external_mate) {
        free(first_port);
        Cap_free(cap);
        return NULL;
...
static int compute_jordan_wigner_sign(int src_res, int dst_res)
{
    ...
    return (pop_count((unsigned int)prior_ones) % 2 == 0) ? 1 : -1;
}
...
KhovanovKomplex *KhovanovKomplex_alloc(Knot *knot)
{
    ...
    kh_complex->states_count = (int *)calloc((size_t)(m + 1), sizeof(int));
    kh_complex->states_by_h = (int **)calloc((size_t)(m + 1), sizeof(int *));
    kh_complex->columns = (SmoothingColumn **)calloc((size_t)(m + 1), sizeof(SmoothingColumn *));
    ...
    for (int r = 0; r < num_states; r++) {
        kh_complex->states_count[pop_count((unsigned int)r)]++;
    }

    int *current_idx = (int *)calloc((size_t)(m + 1), sizeof(int));
    if (!current_idx)
        goto alloc_failure;

    for (int h = 0; h <= m; h++) {
        if (kh_complex->states_count[h] > 0) {
            kh_complex->states_by_h[h] =
                (int *)malloc((size_t)kh_complex->states_count[h] * sizeof(int));
    ...
    for (int h = 0; h <= m; h++) {
        ...
        kh_complex->columns[h]->numbers =
            (int *)calloc((size_t)kh_complex->columns[h]->n, sizeof(int));
        kh_complex->columns[h]->smoothings =
            (Cap **)malloc((size_t)kh_complex->columns[h]->n * sizeof(Cap *));

        if (!kh_complex->columns[h]->numbers || !kh_complex->columns[h]->smoothings)
            goto alloc_failure;
...
bool KhovanovKomplex_build(KhovanovKomplex *kh_complex)
{
    ...
    for (int h = 0; h < m; h++) {
        CobMatrix *boundary =
            CobMatrix_create(kh_complex->columns[h], kh_complex->columns[h + 1], false);
        if (!boundary)
            return false;
